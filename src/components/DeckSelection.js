...
export default function DeckSelection({ webOfTeam, userDeckData, selectedDeck, showCharactersInSelectedDeck, handleShowCharactersInSelectedDeck }) {
  ...
  async function handleAddTeamToDeck() {
    ...
    await addTeamToDeck({
      ...
      .then((result) => {
        // console.log(result.data.addTeamToDeck._id);
      })
      .catch((error) => {
        setErrorMessage(error.message);
        setOpenErrorModal(true);
  ...
  return (
    ...
    <div className="flex flex-wrap px-2 justify-center items-center card-sm:justify-between">
      <form className='flex flew-wrap mt-2 card-sm:mt-0 card-sm:w-[30%]' onSubmit={handleRenameDeck}>
      ...
      <button disabled={webOfTeam.length < 6 || webOfTeam.length >= 8} onClick={() => handleAddTeamToDeck()} className="disabled:bg-gray-500 flex justify-center items-center w-[65%] h-12 my-2 border-4 border-black text-md card-sm:text-lg font-bold rounded-full bg-orange-200 hover:bg-orange-400 transition ease-in-out">
        ...
        {allTeams ? allTeams.map(team => (
          <div key={team._id} className='relative'> 
            ...
            <div className="flex flex-wrap w-full h-1/4 py-2 px-8 mb-2 border-x-4 border-b-4 border-black rounded-b-lg justify-around bg-orange-200 relative">
              {/* lazy sorting but it works */}
              {team.characters.map((individualCharacter => (
                  (team.info.leader === individualCharacter.id) ?
                      <CharacterCard individualCharacter={individualCharacter} team={team} /> : null
                  )))}
              {team.characters.map((individualCharacter => (
...
const CharacterCard = ({individualCharacter, team}) => {
  ...
  return (
    <>
        <div className='w-fit relative'>
          {individualCharacter.id === team.info.leader ? <img src={leaderIcon} className='w-[56px] card-sm:w-[80px] -top-[2%] right-[33%] absolute z-50'/> : null}
          {individualCharacter.id === team.info.subLeader ? <img src={subLeaderIcon} className='w-[56px] card-sm:w-[80px] -top-[2%] right-[33%] absolute z-50'/> : null}
          {individualCharacter.id === team.info.leader && individualCharacter.id === team.info.subLeader ?
            <>
              <img src={leaderIcon} className='w-[56px] card-sm:w-[80px] -top-[2%] right-[33%] absolute z-50'/>
              <img src={subLeaderIcon} className='w-[56px] card-sm:w-[80px] -top-[18%] right-[18%] absolute z-40'/> 
            </>
          : 
            null}
          <AdvancedImage
